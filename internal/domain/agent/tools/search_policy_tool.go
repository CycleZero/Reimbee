...
type SearchPolicyInput struct {
	Query string `json:"query" jsonschema:"required" jsonschema_description:"检索查询（自然语言，如'差旅住宿费报销标准'）"`
	Limit int    `json:"limit" jsonschema:"default=5" jsonschema_description:"返回结果数上限，默认5条"`
}
...
// SearchPolicyTool 命名类型（Wire DI 区分类键）
