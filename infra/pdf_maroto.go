...
// 中文字体文件路径（按优先级查找，完整CJK字体优先于替补字体）
var chineseFontPaths = []string{
	"./data/simhei.ttf",                                          // 项目内置（将 .ttf 放入此路径即可）
	"/usr/share/fonts/truetype/wqy/wqy-zenhei.ttc",               // Linux 文泉驿正黑（完整CJK）
	"/usr/share/fonts/opentype/noto/NotoSansCJK-Regular.ttc",     // Linux Noto（完整CJK）
	"/usr/share/fonts/truetype/wqy/wqy-microhei.ttc",             // Linux 文泉驿微米黑
	"/usr/share/fonts/truetype/droid/DroidSansFallbackFull.ttf",   // Linux Droid（替补字体，CJK不全）
